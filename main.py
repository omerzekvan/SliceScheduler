...
oneNodeCPU = 100
originalNodeCapacities = sorted([oneNodeCPU, oneNodeCPU, oneNodeCPU, oneNodeCPU, oneNodeCPU, oneNodeCPU],
                                reverse=True)  # , oneNodeCPU, oneNodeCPU, oneNodeCPU, oneNodeCPU], reverse=True)
nodeCapacity = sorted([oneNodeCPU, oneNodeCPU, oneNodeCPU, oneNodeCPU, oneNodeCPU, oneNodeCPU],
                      reverse=True)  # , oneNodeCPU, oneNodeCPU, oneNodeCPU, oneNodeCPU], reverse=True)
N = len(nodeCapacity)
...
def rateSlices(ratinglevel):
    if ratinglevel == 0:
        ...
        for l in sliceRequests:
            ...
            for s in l["services"]:
                l["points"] += 10 ** 3 * functionsCatalog[s]["cpu"] + (
                            functionsCatalog[s]["reqCount"] + functionsCatalog[s]["lowReqCount"]) / size
    elif ratinglevel == 4:
        for l in sliceRequests:
            size = len(l["services"])
            # prior = 10 if l["priority"] == 1 else 2
...
if __name__ == '__main__':
    ...
    try:
        ...
        with open("results.txt", "a") as file1, open("usage.txt", "a") as file2, open("satisfied.txt",
                                                                                      "a") as file3, open(
                "guests.txt", "a") as file4, open("timeLine.txt", "a") as file5, open("scores.txt", "a") as file6:

            file1.write(
                "NFavailability = {}. 2 pods are onboard if HA({}) is required else only 1 pod is onboard\n".format(
                    NFavailability, HighAv))
            file2.write(
                "NFavailability = {}. 2 pods are onboard if HA({}) is required else only 1 pod is onboard\n".format(
                    NFavailability, HighAv))
            file3.write(
            ...
            for experiment in range(0, numberOfExperiments):
                ...
                for control in range(0, controlGroups - 1):
                    ...
                    for r in sortedSlices:
                        ...
                        for s in r["services"]:
                            ...
                            if not foundT:
                                ...
                                for f in functionsList:
                                    ...
                                    functionId = db.insertFunction(functionsCatalog[f]["name"], cpu, av, [],
                                                                   new_service_id)
                        ...
                        if not sliceFailed:
                            # DB'deki Slice aktifliği kullanılmıyor.
                    ...
                    underUtil = sum(nodeCapacity) / sum(originalNodeCapacities)
                    ...
                    if control == 2 or control == 9 or control == 10:
                        ...
                        if maxSatisfiedRequestsInExperimentAmongFavs < satisfiedRequests:
                            ...
                            bestResultUnderUtilAmongFavs = underUtil
                    ...
                    outputs.append(
                        "Control Set: {} Total Number of requests: {} Number of satisfied requests: {} Number of guests: {} Average Utilization: {}".format(
                ...
                sumOfUnderUtil[controlGroups - 1] += bestResultUnderUtilAmongFavs
                ...
                outputs.append(
                    "Control Set: {} Total Number of requests: {} Number of satisfied requests: {} Number of guests: {} Average Utilization: {}".format(
                        controlGroups - 1, numberOfReqs, maxSatisfiedRequestsInExperimentAmongFavs,
                        bestResultNumberOfGuestSlicesAmongFavs, bestResultAvrgUtilAmongFavs))

            with open("results.txt", "a") as file1, open("usage.txt", "a") as file2, open("satisfied.txt",
                                                                                          "a") as file3, open(
                    "guests.txt", "a") as file4, open("timeLine.txt", "a") as file5, open("scores.txt",
                                                                                          "a") as file6, open(
                    "underutil.txt", "a") as file7:
                ...
                for c in range(0, controlGroups):
                    ...
                    avrgUnderUtil[c] = round(sumOfUnderUtil[c] / numberOfExperiments, 2)
